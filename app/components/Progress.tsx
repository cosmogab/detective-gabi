  ...
  return (
    <div>
      ...
      <div className="relative mt-10 flex h-20 overflow-hidden border border-rule-strong bg-card sm:h-24">
        ...
        {/* The step twice, in ink over the paper and in paper clipped to the ink, so the word
            stays legible wherever the fill has reached. One string, two colours, no blend. */}
        {step !== undefined ? (
          <>
            <span
              aria-hidden="true"
              className="absolute inset-0 flex items-center px-4 text-ink sm:px-5"
            >
              <Step name={SOURCE_NAME[step]} running={running && !done} />
            </span>
            <span
              aria-hidden="true"
